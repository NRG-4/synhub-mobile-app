...
interface AnalyticsWebService {
    ...
    suspend fun getTaskOverview(@Path("groupId") groupId: Long): AnalyticsResponse

    @GET("api/v1/metrics/tasks/distribution/{groupId}")
    suspend fun getTaskDistribution(@Path("groupId") groupId: Long): AnalyticsResponse

    @GET("api/v1/metrics/tasks/rescheduled/{groupId}")
    suspend fun getRescheduledTasks(@Path("groupId") groupId: Long): AnalyticsResponse

    @GET("api/v1/metrics/tasks/avg-solution-time/{leaderId}")
    suspend fun getAvgSolutionTime(@Path("leaderId") leaderId: Long): AnalyticsResponse

    @GET("api/v1/metrics/tasks/avg-dev-time/{memberId}")
    suspend fun getAvgDevTime(@Path("memberId") memberId: Long): AnalyticsResponse

    @GET("api/v1/metrics/task/{taskId}/time-passed")
    suspend fun getTaskTimePassed(@Path("taskId") taskId: Long): TaskTimePassedResponse

    @GET("api/v1/metrics/groups/{groupId}/member-count")
    suspend fun getGroupMemberCount(@Path("groupId") groupId: Long): GroupMemberCountResponse
}
